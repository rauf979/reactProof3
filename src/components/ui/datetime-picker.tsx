...
function getValidArrowNumber (value: string, { min, max, step }: GetValidArrowNumberConfig): string {
  let numericValue = parseInt(value, 10)
  if (!isNaN(numericValue)) {
    numericValue += step
...
function setDateByType (date: Date, value: string, type: TimePickerType, period?: Period): Date {
  switch (type) {
    ...
    case '12hours': {
      if (period === undefined) return date
...
function Calendar ({
  ...
  yearRange = 50,
  ...props
}: CalendarProps & { yearRange?: number }): JSX.Element {
        ...
        CaptionLabel: ({ displayMonth }) => {
          return (
            <div className='inline-flex gap-2'>
              ...
              >
                <SelectTrigger className='p-0 border-none w-fit focus:bg-accent focus:text-accent-foreground'>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {MONTHS.map((month) => (
  ...
  (
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (['ArrowUp', 'ArrowDown'].includes(e.key)) {
        ...
        const tempDate = (date != null) ? new Date(date) : new Date()
...
const TimePicker = React.forwardRef<TimePickerRef, TimePickerProps>(
  ({ date, onChange, hourCycle = 24, granularity = 'second' }, ref) => {
    ...
    const [period, setPeriod] = React.useState<Period>((date != null) && date.getHours() >= 12 ? 'PM' : 'AM')
